...
public class TxtExportByDaySort {
...
//                }
        ...
        //把文件以日期做分类，归类rusultFileObjects集合
        Map<String, List<FileObject>> fileMapGroupByYMDDate = fileObjectsTemp.stream().collect(Collectors.groupingBy(FileObject::getFileYMDDate));
        //把文件夹List转为map key为文件夹全路径
        Map<String, FileObject> dirMapGroupByYMDDate = dirObjects.stream().collect(Collectors.toMap(FileObject::getParentDir, t -> t,(k1,k2)->k1));

        //把文件放对应文件夹中
        Map<String, Map<FileObject, List<FileObject>>> dateListMap = new HashMap<>();
        List<List<FileObject>> rusultFileObjects = new ArrayList<>();
        for (Map.Entry<String, List<FileObject>> entry : fileMapGroupByYMDDate.entrySet()) {
            if(!dateListMap.containsKey(entry.getKey())){
                Map<FileObject, List<FileObject>> dateMap = new HashMap<>();
                dateListMap.put(entry.getKey(), dateMap);
            }

            Map<FileObject, List<FileObject>> dateMapTemp = dateListMap.get(entry.getKey());
            for (Map.Entry<String, FileObject> entryDir : dirMapGroupByYMDDate.entrySet()) {
                for(FileObject fileObject : entry.getValue()){
                    if(fileObject.getParentDir().equals(entryDir.getValue().getParentDir())){
                        if(!dateMapTemp.containsKey(entryDir.getValue())){
                            List<FileObject> dateList = new ArrayList<>();
                            dateMapTemp.put(entryDir.getValue(), dateList);
                        }
                        dateMapTemp.get(entryDir.getValue()).add(fileObject);
                    }
                }
...
//        //排序
//        List<Map.Entry<String, Map<FileObject, List<FileObject>>>> list2 = new ArrayList<>();
//        list2.addAll(dateListMap.entrySet());
//        if (order != null && order.equals(" 正序 - 所属文件夹内排序")) {
//            Collections.sort(list2, (o1, o2) -> o1.getKey().compareTo(o2.getKey()));
//        } else {
//            Collections.sort(list2, (o1, o2) -> o2.getKey().compareTo(o1.getKey()));
//        }
//        list2.forEach(entry -> {
...
//        });
        ...
        // 按照Map的键进行排序
        Map<String, Map<FileObject, List<FileObject>>> dateListSortedMap = dateListMap.entrySet().stream()
                .sorted(order != null && order.equals(OrderType.ORDER_TYPE_ASC.getIndex())
                        ?
                        Map.Entry.<String, Map<FileObject, List<FileObject>>>comparingByKey()
                        :
                        Map.Entry.<String, Map<FileObject, List<FileObject>>>comparingByKey().reversed())
                .collect(
                        Collectors.toMap(
                                Map.Entry::getKey,
                                Map.Entry::getValue,
        ...
        //转为List
        for (Map.Entry<String, Map<FileObject, List<FileObject>>> mapEntry : dateListSortedMap.entrySet()) {
            List<FileObject> fileObjectList = new ArrayList<>();
            FileObject workDateObject = new FileObject();
            workDateObject.setType(FileType.FILE_TYPE_WORK_DATE.getName());
            workDateObject.setFileYMDDate(mapEntry.getKey());
            fileObjectList.add(workDateObject);
            // 按照Map的键进行排序
            Map<FileObject, List<FileObject>> dirListSortedMap = mapEntry.getValue().entrySet().stream()
                    .sorted(Map.Entry.comparingByKey(order != null && order.equals(OrderType.ORDER_TYPE_ASC.getIndex())
                            ?
                            Comparator.comparing(FileObject::getCreateDate)
                            :
                            Comparator.comparing(FileObject::getCreateDate).reversed()))
                    .collect(
                            Collectors.toMap(
                                    Map.Entry::getKey,
                                    Map.Entry::getValue,
            ...
            for (Map.Entry<FileObject, List<FileObject>> entry : dirListSortedMap.entrySet()) {
                FileObject folder = (FileObject) BeanUtils.cloneBean(entry.getKey());
                folder.setDirInnerFileNum(entry.getValue().size());
                fileObjectList.add(folder);
                List<FileObject> fileList = null;
                if (order != null && order.equals(OrderType.ORDER_TYPE_ASC.getIndex())) {
                    fileList = entry.getValue().stream().sorted(Comparator.comparing(FileObject::getLastModifiedDate))
                            .collect(Collectors.toList());
                }else {
                    fileList = entry.getValue().stream().sorted(Comparator.comparing(FileObject::getLastModifiedDate).reversed())
                            .collect(Collectors.toList());
                }
                AtomicInteger innerOrder = new AtomicInteger(1);
                fileList.forEach(vo -> {
                    vo.setFileInnerOrder(innerOrder.getAndIncrement());
                });
                fileObjectList.addAll(fileList);
            }
            rusultFileObjects.add(fileObjectList);
        }
    ...
    public static List<FileObject> getAllFile(String dirPath, List<FileObject> fileObjects) throws Exception {
        ...
        for (File f : files) {
            ...
            if (f.isFile() && f.getName().endsWith(".txt")) {
                ...
                //类型
                fileObject.setType(FileType.FILE_TYPE_DOC.getName());
                //大小
            ...
            if (f.isDirectory()) {
                fileObject = new FileObject();
                //类型
                fileObject.setType(FileType.FILE_TYPE_FOLDER.getName());
                //路径
...
//		            code = "Not UTF-8";
    ...
    public static void exportAsc() {
        export(OrderType.ORDER_TYPE_ASC.getIndex(), TagType.TAG_TYPE_SORT_BY_WORK_DAY.getName(), Contents.SOURCE_FILES_ROOT_DIR, Contents.EXPORT_FILE_NAME);
    }

    public static void exportDesc() {
        export(OrderType.ORDER_TYPE_DESC.getIndex(), TagType.TAG_TYPE_SORT_BY_WORK_DAY.getName(), Contents.SOURCE_FILES_ROOT_DIR, Contents.EXPORT_FILE_NAME);
    }
